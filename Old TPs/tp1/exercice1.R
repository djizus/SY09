...
boxplot(a, b, main = "Comparaison du poids des bébés à la naissance",  col = c("white", "red"), names = c("Fumeuses", "Non Fumeuses"), ylab = "bwt", notch = TRUE)
dev.off()
...
boxplot(c, d, main = "Comparaison du temps de gestation des mères", col = c("white", "red"), names = c("Fumeuses", "Non Fumeuses"), ylab = "gestation", notch = TRUE)
dev.off()
...
barplot(rbind(NonSmoke, Smoke), space = 0,legend = levels('Smoke'), main = "Comparaison du niveau d'études des mères", col = c('white','red'), xlab = "niveau d'étude", ylab = "nombre d'individus", legend.text = T)
dev.off()
